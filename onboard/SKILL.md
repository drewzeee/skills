...
Do NOT create README.md files in each folder. VAULT.md is the single
source of truth for what goes where — folder-level READMEs just
duplicate that information and create maintenance burden. The only
exception is family/README.md, which holds actual data (household
overview table, upcoming dates) rather than just a folder description.
...
conventions in the CLAUDE.md system instructions — just point to
...
4. A rule to update VAULT.md when the vault structure or organizational
   conventions change.
5. A rule to update family/README.md and family/{person}.md files when
   relevant new information comes up.
6. A reminder that most updates should just happen silently — don't ask
   permission for routine maintenance.

The system instructions should NOT duplicate the vault structure, file
creation rules, or naming conventions inline. Instead, point the agent
to VAULT.md as the single source of truth for all organizational
details. This keeps the system instructions focused on behavior and
avoids two files drifting out of sync.